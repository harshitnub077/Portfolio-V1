...
const PROJECT_STYLES = {
  ...
  PROJECTS_WRAPPER:
    "mt-6 grid grid-flow-col auto-cols-max md:gap-10 gap-6 project-wrapper w-fit seq snap-x scroll-pl-6 snap-mandatory pb-12",
};
...
const ProjectsSection = ({ isDesktop }: IDesktop) => {
  ...
  const initProjectsAnimation = (
    ...
    const scrollTrigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        gsap.to(targetSectionRef.current.querySelectorAll(".project-tile"), {
          skewX: skew,
          overwrite: "auto",
          duration: 0.1,
          ease: "power2.out",
  ...
  const renderSectionTitle = (): React.ReactNode => (
    ...
    >
      ...
      <h1 className="section-heading seq mt-2">The Gallery</h1>
      <h2 className="text-2xl md:max-w-3xl w-full seq max-w-sm mt-2">
        A curated selection of digital artifacts and experiences.
      </h2>
    </div>
  );
  ...
  const renderProjectTiles = (): React.ReactNode =>
    PROJECTS.map((project) => (
      <div key={project.name} className="project-tile origin-bottom transition-transform will-change-transform">
        <ProjectTile
          project={project}
